...
export default async function handler(req, res) {
  ...
  try {
    ...
    const question  = sanitize(body.question, 2000);
    ...
    const record = { id, ts, lastName, firstName, middleName, contact, question };

    const redis = await getRedis();
    await redis.lPush('questions', JSON.stringify(record));
    await redis.lTrim('questions', 0, 9999);

    const text =
      `❓ <b>Вопрос Андрею Вертнякову</b>\n` +
      `${escHtml([lastName, firstName, middleName].filter(Boolean).join(' '))}\n` +
      `📞 ${escHtml(contact)}\n` +
      `🕐 ${ts}\n\n` +
      `<i>${escHtml(question)}</i>`;
    await notifyAdmins(text);

    res.status(200).json({ ok: true, id });
  } catch (e) {
    console.error('question err', e);
